...
def run_integration_tests():
    ...
    # Run integration tests
    result = run_command(
        "python -m pytest tests/ -m integration -v", capture_output=True
    )
    if result is None:
...
def run_linting():
    ...
    try:
        ...
        result = run_command(
            "flake8 tests/ examples/ --max-line-length=100", capture_output=True
        )
        if result is None:
...
def main():
    ...
    parser.add_argument(
        "--examples", action="store_true", help="Run only example tests"
    )
    ...
    if not any([args.python, args.integration, args.examples, args.lint, args.format]):
        args.all = True

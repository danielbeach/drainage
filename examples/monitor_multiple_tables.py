...
def monitor_tables(tables: List[Tuple[str, str]], aws_region: str = "us-west-2"):
    ...
    print(f"\n{'='*80}")
    print("Data Lake Health Monitoring")
    print(f"{'='*80}\n")
            ...
            results.append(
                {
                    ...
                    "unreferenced_size_gb": report.metrics.unreferenced_size_bytes
                    / (1024**3),
    ...
    if successful_results:
        ...
        avg_health = sum(r["health_score"] for r in successful_results) / len(
            successful_results
        )
    ...
    if unhealthy_tables:
        print("⚠️  Tables Needing Attention:")
        print(f"{'─'*80}")
